...
Zie [Voordelen van at.js voor meer informatie over de verschillen tussen de twee Target Javascript-bibliotheken](/help/c-implementing-target/c-implementing-target-for-client-side-web/t-mbox-download/c-target-atjs-implementation/target-atjs-implementation.md#benefits).
...
|--- |--- |
| Aangepaste globale mabox | (Optioneel) Geef de aangepaste globale box op die wordt gebruikt voor [!DNL Target] activiteiten. Standaard wordt de algemene box gebruikt door [!DNL Target} voor [!DNL Recommendations].<br>Opmerking: Deze optie is ingesteld op de [!DNL Target] [!UICONTROL Administration] pagina. Open [!DNL Target], klik dan [!UICONTROL Administration] > [!UICONTROL Visual Experience Composer]. |
...
| Token voor aanbevelingen-API | Gebruik dit token in API-aanroepen van Aanbevelingen, zoals de API voor downloaden. |
