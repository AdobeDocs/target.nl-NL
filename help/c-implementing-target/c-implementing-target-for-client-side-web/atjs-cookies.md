...
Voor at.js versie 2.0.0 worden *alleen cookies van eerste bedrijven ondersteund*. Net als in om.js 1.*x*, het eerste koekje, &quot;mbox,&quot;wordt opgeslagen in `clientdomain.com`, waar `clientdomain` is uw domein.

at.js produceert een zitting - identiteitskaart en slaat het in het koekje op. De eerste reactie bevat alle informatie over de activiteit, evenals de activiteit `TNT` of `PC ID` die door de [!DNL Target] servers is gegenereerd. at.js schrijft de cookie vervolgens `TNT/PC ID` naar het cookie.

Het cookie van de `AMCV_###@AdobeOrg` eerste partij wordt altijd ingesteld door de gebruiker, maar de cookie [!DNL Experience Cloud ID Service]wordt doorgegeven in `ECID` [!DNL Target] aanvragen.
...
at.js produceert een `mboxSession ID` en slaat het in het koekje op. De eerste reactie bevat de aanbieding en de JavaScript-code om de `mboxPC ID` gegenereerde code in het cookie op te slaan.

>[!NOTE]
>
>Het cookie van de `AMCV_###@AdobeOrg` eerste partij wordt altijd ingesteld met het [!DNL Experience Cloud Visitor ID]pictogram.
