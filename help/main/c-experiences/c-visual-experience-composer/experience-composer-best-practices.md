...
De volgende best practices kunnen u helpen uw ervaringen naar behoren te laten werken. Er zijn ook andere uiteinden en beperkingen die u van bewust zou moeten zijn wanneer het gebruiken van Composer van de Visuele Ervaring (VEC) in [!DNL Adobe Target].
...
**Plaats de verwijzing at.js bij de bovenkant van `<head>` van uw pagina.**
...
Als u de Enhanced Experience Composer op accountniveau wilt inschakelen, klikt u op [!UICONTROL Administration > Visual Experience Composer]en schakelt u vervolgens over naar de Aan-positie.

Om de Verbeterde Composer van de Ervaring op het activiteitenniveau toe te laten terwijl het creëren van een activiteit in de Visuele Composer van de Ervaring [!UICONTROL Configure > URL]en schakelt u vervolgens over naar de Aan-positie.
...
Gebruik de opdracht `!important` CSS-eigenschap bij het verbergen of verwijderen van elementen.
...
Zie [Elementkiezers die worden gebruikt in de composer voor visuele ervaring](/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337) .

**Gebruiken `<b>` en `<i>` tags bij het opmaken van tekstelementen met de RTF-editor.**

* Voor vette tekst: gebruiken `<b>` eerder dan `<strong>`.
* Gebruik voor cursieve tekst `<i>` eerder dan `<em>`.
...
**Niet opnemen `mboxCreate` in scripts.**

Omdat `mboxCreate` gebruik `document.write`wordt niet aanbevolen `mboxCreate` in scripts. Gebruik in plaats daarvan `mboxDefine` en `mboxUpdate` voor hetzelfde doel.
...
**Niet gebruiken `top` of `self` variabelen in JavaScript-code.**
...
Omdat er geen Z-indexfunctionaliteit is, kan het verplaatste element niet boven op een ander element worden geplaatst. Zie [Beperkingen](/help/main/c-experiences/c-visual-experience-composer/experience-composer-best-practices.md#section_F33C2EA27F2E417AA036BC199DD6C721) voor meer informatie .
...
**Het invoegen van een element werkt mogelijk niet in een `<div>` dat is een mbox .**
