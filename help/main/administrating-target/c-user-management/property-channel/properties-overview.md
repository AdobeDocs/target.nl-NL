...
U moet de werkruimte-id doorgeven aan Enterprise-machtigingen in [Doel-API&#39;s](https://experienceleague.corp.adobe.com/docs/target-dev/developer/server-side/server-side-overview.html){target=_blank}.
...
1. Werk uw [!DNL Target] uitvoering met de in de vorige stap verkregen uitvoeringscode.
   ...
   * **Via de functie targetPageParamsAll():** Plaats de volgende code in de `<head>` -tags, boven de referentie at.js.
      ...
      Ga voor meer informatie over hoe u dit kunt doen met at.js naar [targetPageParamsAll](https://experienceleague.corp.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetpageparamsall.html){target=_blank}.
