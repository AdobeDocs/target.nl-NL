...
Op deze pagina worden de gedetailleerde statistische berekeningen beschreven die in de handmatige A/Bn-tests in [!DNL Adobe Target]. Er worden definities gegeven voor Omzettingssnelheid, Vertrouwensinterval van Omzetsnelheid, Lift, Vertrouwensinterval voor Lift en Vertrouwen.

![Het rapport van het doel dat het Tarief van de Omzetting, Gemiddelde Ophef en het Interval van het Vertrouwen, en het Vertrouwen van een activiteit van de Test A/B toont.](/help/main/c-reports/statistical-methodology/img/target_report.png)
...
* De som over eenheden *i* hangt af van de keuze van de telmethode.
   ...
   * Indien *Bezoekers* wordt gebruikt als telmethode, is elke eenheid een unieke bezoeker die wordt gedefinieerd als een unieke deelnemer aan de activiteit gedurende de levensduur van de activiteit.
   * Indien *Bezoeken* wordt gebruikt als telmethode, is elke eenheid een uniek bezoek dat wordt gedefinieerd als een unieke deelnemer aan een ervaring tijdens een [!DNL Target] sessie (met een unieke `sessionId`). Wanneer de `sessionId` wanneer de bezoeker de conversiestap bereikt, wordt een nieuw bezoek geteld.
   * Indien *Activiteitenindrukkingen* wordt gebruikt als telmethode, is elke eenheid een unieke indruk die wordt gedefinieerd als elke keer dat een bezoeker een pagina van de activiteit laadt.

## Betrouwbaarheidsinterval tussen gemiddelde/omrekeningsfactor
...
Bij het uitvoeren van experimenten is de omrekeningskoers die we voor een bepaalde ervaring waarnemen een *schatten* van de &quot;werkelijke&quot; omrekeningskoers. Om de onzekerheid in deze schatting te kwantificeren, kunnen we een betrouwbaarheidsinterval gebruiken. [!DNL Target] er wordt altijd een betrouwbaarheidsinterval van 95 % gerapporteerd , wat betekent dat op lange termijn 95 % van de berekende betrouwbaarheidsintervallen de werkelijke omrekeningskoers van de ervaring omvat .
...
De volgende afbeelding toont Lift and Confidentie Interval of Lift in een [!DNL Target] Rapport. Het getal vertegenwoordigt het gemiddelde van het bereik van de liftgrenzen en de pijl geeft aan of de lift positief of negatief is. De pijl wordt grijs weergegeven totdat het vertrouwen 95% bereikt. Als het vertrouwen de drempel bereikt, is de pijl groen of rood op basis van een positieve of negatieve lift.

<p style="text-align:center;"><img width="35%" src="img/lift.png"></p>

De lift tussen twee ervaringen  *ν* en de controleervaring *ν<sub>0</sub>* is de relatieve &quot;delta&quot; in omrekeningskoersen, gedefinieerd als
...
## Vertrouwelijk interval van optillen

De grafiek van het klokveld in de [!UICONTROL Average Lift and Confidence Interval] de kolom vertegenwoordigt de gemiddelde waarde en 95% het Interval van het Vertrouwen van Lift. Het veld is grijs wanneer het betrouwbaarheidsinterval van een gegeven ervaring met niet-besturing overlapt met het betrouwbaarheidsinterval van de controleervaring en groen of rood is wanneer het betrouwbaarheidsinterval van de ervaring groter of kleiner is dan het betrouwbaarheidsinterval van de controleervaring.
...
Ten slotte wordt in de [!DNL Target] wordt gedefinieerd als:
